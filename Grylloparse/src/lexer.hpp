...
struct LexicToken{
    ...
    const static int END_OF_STREAM_TOKEN = -2;
...
public:
     ...
     */ 
    Lexer( const RegLexData& lexicData, std::istream& stream, bool useBlockingQueue = false );
    Lexer( RegLexData&& lexicData, std::istream& stream, bool useBlockingQueue = false );
    ~Lexer(){}
